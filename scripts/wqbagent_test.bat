...
:: pwsh -Command "$OutputEncoding = [Console]::OutputEncoding = [System.Text.Encoding]::UTF8; python -u \"%PARENT_DIR%\wqbagent_test.py\" | ForEach-Object { $_ | Out-Host; $_ } | ansi2html > \"%OUTPUT_FILE%\""

:: Terminal worked but the output html was blank.
@REM pwsh -Command "$transcript = '%TRANSCRIPT%'; Start-Transcript -Path $transcript -Force -IncludeInvocationHeader:$false; python -u \"%PARENT_DIR%\wqbagent_test.py\"; Stop-Transcript; Get-Content -Path $transcript -Raw -Encoding utf8 | ansi2html > \"%OUTPUT_FILE%\"; Remove-Item $transcript -ErrorAction SilentlyContinue"

:: === FINAL FIXED BLOCK (now works because TRANSCRIPT is defined) ===
pwsh -Command "$transcript = $env:TRANSCRIPT; $outputFile = $env:OUTPUT_FILE; Start-Transcript -Path $transcript -Force -IncludeInvocationHeader:`$false; python -u \"%PARENT_DIR%\wqbagent_test.py\"; Stop-Transcript; Get-Content -Path $transcript -Raw -Encoding utf8 | ansi2html > \"$outputFile\"; Remove-Item $transcript -ErrorAction SilentlyContinue"
